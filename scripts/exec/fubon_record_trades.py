...
# ---------------------------------------------------------------------
# sys.path bootstrap: allow "python scripts/exec/fubon_record_trades.py"
# ---------------------------------------------------------------------
...
def _open_output(out_root: str, date: str, run_id: str) -> Path:
    out_dir = Path(out_root) / "trades" / f"dt={date}"
    out_dir.mkdir(parents=True, exist_ok=True)
    return out_dir / f"trades_{run_id}.ndjson"
...
def _invoke_trade_api(stock: Any, account: Any, start: str, end: str) -> Any:
    fn = getattr(stock, "filled_history", None)
    if not callable(fn):
        raise RuntimeError("FUBON_TRADE_API_NOT_FOUND: filled_history")
    return _call_with_variants(fn, account, start, end)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Record Fubon trades to NDJSON (poll-only).")
...
def _build_records(
    *,
    ...
    end_key = start_key
    resp = _invoke_trade_api(stock, account, start_key, end_key)
    ...
    try:
        for date in dates:
            ...
            try:
                ...
                print(f"FILLED_HISTORY_FAIL date={date} error={exc}")
                return EXIT_EXCEPTION

        print(f"OK trades_written={total} dates={len(dates)}")
        return EXIT_OK
    finally:
        fubon_provider.close(ctx)
