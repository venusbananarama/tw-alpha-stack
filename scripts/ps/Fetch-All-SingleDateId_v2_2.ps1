...
.SYNOPSIS
  ...
  - PowerShell 7 ForEach-Object -Parallel（避免 Job/ThreadJob 相容問題）
  - 全域速率保護：ThrottleLimit × QpsPerWorker ≤ 0.16（FinMind 600/hr）
  - 先跑「市場級」(Shim)；再跑單股（逐 dataset 打點、重試、每檔獨立 log）
  - 支援 smoke test（-MaxSymbols）、強制清除 Resume、Sequential 模式（-Sequential）
#>
...
# 定義每檔工作內容（逐 dataset）
$perSymbol = {
  param($sym,$dsSingle,$Start,$End,$QpsPerWorker,$ResumeLog,$effectiveToken,$LogsDir)

  $InvokeSingle = Join-Path $using:PSScriptRoot 'Invoke-FMSingle.ps1'
  ...
  foreach ($ds in $dsSingle) {
    ...
    while ($true) {
      try {
        $attempt++
        & $InvokeSingle -Symbol $sym -Datasets @($ds) -Start $Start -End $End -Qps $QpsPerWorker -VerboseCmd `
...
if ($Sequential) {
  foreach ($sym in $symbols) {
    ...
    & $perSymbol $sym $ds_single $Start $End $QpsPerWorker $ResumeLog $effectiveToken $LogsDir
  }
} else {
  ...
  $symbolsToRun | ForEach-Object -Parallel $perSymbol -ThrottleLimit $ThrottleLimit -ArgumentList @(
    $ds_single,$Start,$End,$QpsPerWorker,$ResumeLog,$effectiveToken,$LogsDir
  )
}
