<#
.SYNOPSIS
  v2.1b2 hotfix：修正呼叫 Invoke-FMAll.ps1 的 -Datasets 型別，改傳陣列 @($g) 以符合 .Count 的使用。
  同時保留 v2.1b 的三大功能：速率保護、市場資料集自動分流、symbol 正規化。
#>
...
# 市場級映射 → Invoke-FMAll 的 ValidateSet 群組
$MarketMap = @{
...
# === 市場級先跑（用群組呼叫 FMAll；-Datasets 傳陣列） ===
