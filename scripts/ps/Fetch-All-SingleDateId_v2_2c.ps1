...
.SYNOPSIS
  ...
  - PowerShell 7 ForEach-Object -Parallel（避免 Job 相依性）
  - 單一參數集合：-UniverseCsv / -SymbolsTxt 擇一或皆可（至少指定一個）
  - 市場級 datasets 由 Shim 自動處理（prices/chip/stock_info）
  - 單股 datasets 逐檔逐 dataset 回補，含重試、打點、resume
  - 速率保護：ThrottleLimit × QpsPerWorker ≤ MaxRps（預設 0.16 ≈ 600/hr）
#>
