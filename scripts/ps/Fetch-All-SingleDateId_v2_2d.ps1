<#
.SYNOPSIS
  Fetch-All-SingleDateId v2.2d — Full Runner (Parallel hotfix)
  - 修正：ForEach-Object -Parallel 不支援 -ArgumentList，改用 $using: 捕捉外部變數
  - Sequential 路徑維持以顯式參數呼叫，避免 $using: 限制
#>
...
if (-not (Test-Path $FMAllShim))        { throw "Missing script: $FMAll_Shim（請使用打包附帶的 Shim）" }
...
# 定義：Sequential 路徑的 per-symbol 執行函式（用顯式參數）
function Invoke-PerSymbol {
...
if ($Sequential) {
  foreach ($sym in $symbols) {
    ...
    Invoke-PerSymbol -sym $sym -dsSingle $ds_single -Start $Start -End $End -QpsPerWorker $QpsPerWorker -ResumeLog $ResumeLog -effectiveToken $effectiveToken -LogsDir $LogsDir -InvokeSinglePath $InvokeSinglePath
  }
} else {
  $symbolsToRun = $symbols | Where-Object { -not $done.ContainsKey($_) }
  $symbolsToRun | ForEach-Object -Parallel {
    param($sym)
    # 使用 $using: 捕捉外部變數
    $log = Join-Path $using:LogsDir ("single_{0}.log" -f $sym)
    "=== {0} {1} → {2} | Datasets={3} ===" -f $sym,$using:Start,$using:End,($using:ds_single -join ',') | Tee-Object -FilePath $log

    if ($using:effectiveToken) { $env:FINMIND_TOKEN = $using:effectiveToken }

    $hadFail = $false
    foreach ($ds in $using:ds_single) {
      $attempt=0
      $dsStart = Get-Date
      "[DATASET] {0} | {1} | start {2:HH:mm:ss}" -f $sym,$ds,$dsStart | Tee-Object -FilePath $log -Append
      while ($true) {
        try {
          $attempt++
          & $using:InvokeSinglePath -Symbol $sym -Datasets @($ds) -Start $using:Start -End $using:End -Qps $using:QpsPerWorker -VerboseCmd `
            *>&1 | Tee-Object -FilePath $log -Append
          $dsEnd = Get-Date
          $dur = [int](($dsEnd - $dsStart).TotalSeconds)
          "[OK] {0} | {1} | {2}s" -f $sym,$ds,$dur | Tee-Object -FilePath $log -Append
          Add-Content -Path $using:ResumeLog -Value ("{0}|{1}|ok" -f $sym,$ds)
          break
        } catch {
          $msg = $_.Exception.Message
          Add-Content -Path $log -Value ("[WARN] {0} | {1} | attempt={2} | {3}" -f $sym,$ds,$attempt,$msg)
          if ($attempt -ge 3) {
            Add-Content -Path $using:ResumeLog -Value ("{0}|{1}|fail|{2}" -f $sym,$ds,$msg)
            "[FAIL] {0} | {1} | {2}" -f $sym,$ds,$msg | Tee-Object -FilePath $log -Append
            $hadFail = $true
            break
          }
    ...
    if (-not $hadFail) { Add-Content -Path $using:ResumeLog -Value ("{0}|ok" -f $sym) }
    else { Add-Content -Path $using:ResumeLog -Value ("{0}|partial_fail" -f $sym) }
  } -ThrottleLimit $ThrottleLimit -ArgumentList { $_ }
}
