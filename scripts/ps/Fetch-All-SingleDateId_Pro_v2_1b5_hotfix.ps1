<#
.SYNOPSIS
  v2.1b5 hotfix（Dataset-level Logging 版）
  - 單股批次：逐「資料集」執行與打點（每個 dataset 獨立重試 / 完成即打勾 ✓）
  - 直播輸出：所有子任務輸出即時顯示 + 另存 per-symbol log
  - 保留：速率保護（Workers×Qps ≤ MaxRps）、市場級自動分流（Shim）、symbol 正規化、斷點續跑
...
param(
  ...
  [double]$Qps = 0.04,   # 建議 4×0.04=0.16（600/hr）
...
if ($agg -gt $MaxRps) {
  throw ("總速率超過限制：Workers×Qps={0} > MaxRps={1}（FinMind ≈ 600/hr）" -f $agg, $MaxRps)
}
...
if (-not (Test-Path $FMAllShim))    { throw "Missing script: $FMAllShim（請確保已安裝 v2.1b3+）" }
...
$done=@{}; if (Test-Path $ResumeLog) {
  Get-Content $ResumeLog | % {
    $p=$_.Split('|');
    if ($p.Length -ge 2 -and $p[1]-eq'ok'){$done[$p[0]]=$true}
  }
}
...
$jobScript = {
  param($sym,$Datasets,$Start,$End,$Qps,$ResumeLog,$InvokeSingle,$ApiToken,$MaxRetries,$BackoffSeconds,$LogsDir)
  if ($ApiToken) { $env:FINMIND_TOKEN = $ApiToken }

  $log = Join-Path $LogsDir ("single_{0}.log" -f $sym)
  "=== {0} {1} → {2} | Datasets={3} ===" -f $sym,$Start,$End,($Datasets -join ',') | Tee-Object -FilePath $log
  ...
  foreach ($ds in $Datasets) {
    $attempt=0
    $dsStart = Get-Date
    "[DATASET] {0} | {1} | start {2:HH:mm:ss}" -f $sym,$ds,$dsStart | Tee-Object -FilePath $log -Append | Write-Host
    while ($true) {
      try {
        $attempt++
        # 單一 dataset 執行，直播輸出
        & $InvokeSingle -Symbol $sym -Datasets @($ds) -Start $Start -End $End -Qps $Qps -VerboseCmd `
          *>&1 | Tee-Object -FilePath $log -Append | Write-Host

        $dsEnd = Get-Date
        $dur = [int](($dsEnd - $dsStart).TotalSeconds)
        "[OK] {0} | {1} | {2}s" -f $sym,$ds,$dur | Tee-Object -FilePath $log -Append | Write-Host
        ...
        if ($attempt -ge $MaxRetries) {
          ...
          "[FAIL] {0} | {1} | {2}" -f $sym,$ds,$msg | Tee-Object -FilePath $log -Append | Write-Host
          $hadFail = $true
          break
        }
  ...
  if (-not $hadFail) {
    ...
    Add-Content -Path $ResumeLog -Value ("{0}|partial_fail" -f $sym)
  }
}
...
foreach ($sym in $symbols) {
  ...
  if ($UseThreadJob) {
    $jobs += Start-ThreadJob -Name ("Fetch_{0}" -f $sym) -ScriptBlock $jobScript -ArgumentList $sym,$ds_single,$Start,$End,$Qps,$ResumeLog,$InvokeSingle,$effectiveToken,$MaxRetries,$BackoffSeconds,$LogsDir
  } else {
    $jobs += Start-Job -Name ("Fetch_{0}" -f $sym) -ScriptBlock $jobScript -ArgumentList $sym,$ds_single,$Start,$End,$Qps,$ResumeLog,$InvokeSingle,$effectiveToken,$MaxRetries,$BackoffSeconds,$LogsDir
  }
  $submitted++
}
