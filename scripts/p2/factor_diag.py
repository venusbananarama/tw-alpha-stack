...
使用方式（在 repo root）：
  ...
  # 1) 因子依賴檢查
  python .\\scripts\\p2\\factor_diag.py deps --root . --rules .\\rules_factors.yaml

  # 2) gate 狀態查看
  python .\\scripts\\p2\\factor_diag.py gate --root . \\
      --wf-summary .\\reports\\wf_summary.json \\
      --rules .\\rules_factors.yaml

  # 3) eval 摘要查看
  python .\\scripts\\p2\\factor_diag.py eval --root . \\
