...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            ...
            "This script consumes the JSON produced by scripts/p2/factor_combo.py "
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Output JSON path. Default: <root>/reports/factor_capacity.<as_of>.json",
    )
    p.add_argument(
        "--log-level",
        default="INFO",
    ...
    p.add_argument(
        ...
        help=(
            ...
            "By default, exits 0 as long as the script runs successfully."
        ),
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    combo_plan_path: Path = args.combo_plan or (root / "reports" / f"factor_combo.{as_of}.json")
    combo_plan_path = combo_plan_path.resolve()
    if not combo_plan_path.is_file():
        LOG.error("Combo plan not found: %s", combo_plan_path)
        return 1

    rules_path: Path = (args.rules_file or (root / "rules_factors.yaml")).resolve()

    output_path: Path = args.output or (root / "reports" / f"factor_capacity.{as_of}.json")
    ...
    if plan_as_of != as_of:
        LOG.warning(
            "Mismatch between CLI as-of (%s) and combo plan as_of (%s). Continuing with CLI as-of.",
            as_of,
            plan_as_of,
        )
    ...
    LOG.info(
        "capacity_summary: as_of=%s windows=%s all_pass=%s num_factors=%d",
        summary.as_of,
