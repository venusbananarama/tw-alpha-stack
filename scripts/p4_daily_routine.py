...
def _early_summary_path(as_of: str, run_id: str) -> Path:
    return _REPO_ROOT / "metrics" / "p4_summaries" / f"p4_summary.{as_of}.{run_id}.json"
...
def _write_early_summary(
    *,
    ...
    summary_path = _early_summary_path(as_of, run_id)
    summary = compose_p4_summary(
        as_of=as_of,
        run_id=run_id,
        status=status,
...
def _emit_summary_and_ledger(
    *,
    ...
    append_ledger(
        _REPO_ROOT / "metrics" / "p4_ledger.jsonl",
        {
            "as_of": as_of,
            "run_id": run_id,
            "status": status,
...
def _preflight(
    *,
    ...
    try:
        resolved_exec_trades_path = resolve_exec_trades_path(exec_root, exec_run_id, exec_trades_path)
    ...
    return inputs, resolved_paths, coverage
...
def main() -> int:
    ...
    try:
        trading_days = load_trading_days(_resolve_path(args.calendar_csv))
        ...
        inputs, resolved_paths, coverage = _preflight(
            as_of=as_of,
            exec_root=exec_root,
            exec_run_id=args.exec_run_id,
        ...
        if args.mode in ("all", "replay"):
            ...
            if replay_gate["status"] == "insufficient_data" or impact_gate["status"] == "insufficient_data":
            ...
            if not replay_gate["pass"] or not impact_gate["pass"]:
                status = "FAIL"
        ...
        if args.mode in ("all", "wf"):
            ...
            if wf_gate.get("status") == "insufficient_data":
                if args.on_insufficient_data == "force":
                    status = "WARN"
                    reason_code = REASON_INSUFFICIENT_DATA
                else:
        ...
        if args.mode == "all" and status == "PASS":
            write_ok_flag(_REPO_ROOT / "_state" / "p4" / f"{as_of}.ok")
