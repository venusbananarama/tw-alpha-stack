...
- 完成 factors 組裝後，透過 factor_slo_lib：
...
try:
    from factor_slo_lib import load_factor_slo_config, evaluate_factor_slo
except Exception:  # pragma: no cover
    # 在 unit test 或特殊環境下可能沒有 factor_slo_lib
...
def compose_factors_to_wf(
    ...
    # === C 段微調：沒有任何 eval 檔時，不要動現有 wf_summary ===
    if not eval_files:
        logger.warning(
