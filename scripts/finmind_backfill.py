...
# ===== AlphaCity patch: always emit metrics path (with flush) =====
try:
    _ALPHACITY_METRICS_EMITTED  # guard if upstream already emitted
except NameError:
    _ALPHACITY_METRICS_EMITTED = False

def _alphacity_emit_metrics_fallback():
    import os, sys
    from pathlib import Path

    # 1) 如果上游有定義 metrics_path 且檔案存在，直接輸出（確保 flush）
    mp = globals().get("metrics_path", None)
    if isinstance(mp, (str, os.PathLike)) and os.path.exists(mp):
        print(f"=== Backfill Done ===  metrics: {mp}", flush=True)
        return True

    # 2) 否則回退到專案 metrics 目錄的最新 ingest_summary_*.csv
    try:
        metrics_dir = (Path(__file__).resolve().parents[1] / "metrics")
        if metrics_dir.exists():
            cands = sorted(metrics_dir.glob("ingest_summary_*.csv"), key=lambda p: p.stat().st_mtime, reverse=True)
            if cands:
                print(f"=== Backfill Done ===  metrics: {cands[0]}", flush=True)
                return True
    except Exception:
        pass
    return False

if not _ALPHACITY_METRICS_EMITTED:
    if _alphacity_emit_metrics_fallback():
        _ALPHACITY_METRICS_EMITTED = True
# ===== end AlphaCity patch =====
