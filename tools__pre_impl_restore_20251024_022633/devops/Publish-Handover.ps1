...
if ((Get-Item $zip).Length -ge 2GB) {
  ...
  try{ while($in.Position -lt $in.Length){
    ...
    } finally{ $o.Dispose() }; $i++ } } finally{ $in.Dispose() }
  Remove-Item $zip -Force
  $assets = @(Get-ChildItem $outDir -File | Sort-Object Name | % FullName) + $shaFile
}

if ($ExtraAssets) { $valid = $ExtraAssets | ? { Test-Path $_ }; if ($valid) { $assets += $valid } }
