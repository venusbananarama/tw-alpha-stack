...
# --- FAST params shim (auto-injected, backward compatible) ---
try {
  if(-not [string]::IsNullOrWhiteSpace([string]$Qps)){
    Set-Item -Path Env:FINMIND_QPS -Value ([string]$Qps) | Out-Null
  }
Write-Host ("[FAST] Qps={0} BatchSize={1} MaxConcurrency={2} MaxRetries={3} RetryDelaySec={4}" -f $Qps,$BatchSize,$MaxConcurrency,$MaxRetries,$RetryDelaySec)
} catch {}
# --- end shim ---Set-StrictMode -Version Latest
