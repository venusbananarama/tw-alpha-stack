# tools/maintenance/Clean-Scripts-ToLegacy.ps1
# 用途：把 scripts 底下「非核心」的 .py/.sql/.txt 檔搬到 scripts\legacy\
# 用法：
...
$KeepRelativePaths = @(
  ...
  # Phase-2 combo CLI entry (NO SHIM)
  'p2\factor_combo.py',
  'p2\factor_capacity.py'
)
...
if ($WhatIf) {
  Write-Host "[模式] DRY RUN（只列出搬動清單）" -ForegroundColor Green
} else {
  Write-Host "[模式] 實際搬動檔案" -ForegroundColor Yellow
}

$allFiles = Get-ChildItem -Path $ScriptsDir -Recurse -File |
  Where-Object { $_.Extension -in '.py', '.sql', '.txt' }

$moveList = @()

foreach ($f in $allFiles) {
  $rel = $f.FullName.Substring($ScriptsDir.Length + 1)  # scripts\ 下相對路徑
