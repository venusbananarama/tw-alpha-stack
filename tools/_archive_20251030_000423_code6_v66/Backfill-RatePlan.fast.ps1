...
function Invoke-FinMind([string]$dataset,[string]$ids,[string]$s,[string]$e){
  # 注意：--end 為不含終點；固定帶 --datahub-root 指到根（規範見手冊 §5.3）
  $args = @(''.\scripts\finmind_backfill.py'',''--datasets'',$dataset,''--symbols'',$ids,''--start'',$s,''--end'',$e,''--datahub-root'',$DataRoot)
  Write-Host ">> finmind_backfill $dataset symbols=$ids start=$s end=$e datahub=$DataRoot"
  & $PY @args
...
Write-Host "[OK] RatePlan minimal backfill done. Rules=$Rules Universe=$UniverseFile DataRoot=$DataRoot Start=$Start End=$End" -ForegroundColor Green
