...
# === PF_SAFE_BLOCK_START (auto-injected) ===
$preflightRan = ($pf -ne $null)
$schemaFailCount = 0
$lagViolationCount = 0
if ($preflightRan) {
  $sfProp = $pf.PSObject.Properties['schema_failures']
  if ($sfProp) { $schemaFailCount = @($sfProp.Value).Count }
  $lvProp = $pf.PSObject.Properties['lag_violations']
  if ($lvProp) { $lagViolationCount = @($lvProp.Value).Count }
  # 允許替代欄位名（若存在）
  if (($schemaFailCount -eq 0) -and -not $sfProp) {
    $altSf = $pf.PSObject.Properties['schema_errors']; if ($altSf) { $schemaFailCount = @($altSf.Value).Count }
  }
  if (($lagViolationCount -eq 0) -and -not $lvProp) {
    $altLv = $pf.PSObject.Properties['lag_errors']; if ($altLv) { $lagViolationCount = @($altLv.Value).Count }
  }
}
$preflightOK = $false
if ($preflightRan) {
  if (($pf.PSObject.Properties.Name -contains 'schema_failures') -or
      ($pf.PSObject.Properties.Name -contains 'lag_violations') -or
      ($pf.PSObject.Properties.Name -contains 'schema_errors') -or
      ($pf.PSObject.Properties.Name -contains 'lag_errors')) {
    $preflightOK = ($schemaFailCount -eq 0) -and ($lagViolationCount -eq 0)
  } elseif ($pf.PSObject.Properties.Name -contains 'ok') {
    $preflightOK = [bool]$pf.ok
  } else {
    $preflightOK = $false
  }
}
$freshnessVal = $null
if ($preflightRan -and ($pf.PSObject.Properties.Name -contains 'freshness')) { $freshnessVal = $pf.freshness }
# === PF_SAFE_BLOCK_END ===
