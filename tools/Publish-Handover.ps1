...
$nf = "RELEASE_NOTES_$tag.txt"
("Handover bundle: $tag`r`n`r`nSHA256: $sha`r`n") | Out-File $nf -Encoding utf8

# 連線抖動→最多重試 3 次
$ok=$false; for($i=1;$i -le 3 -and -not $ok;$i++){
  gh release create $tag -R $RepoSlug -t $tag -F $nf --verify-tag --latest $assets
  if($LASTEXITCODE -eq 0){ $ok=$true } else { Start-Sleep 5 }
}
