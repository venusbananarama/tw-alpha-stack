...
$shaFile = "$zip.sha256.txt"; "$sha  $(Split-Path $zip -Leaf)" | Out-File $shaFile -Encoding ascii
...
if ((Get-Item $zip).Length -ge 2GB) {
  ...
  $chunk = $PartMiB * 1MB; $buf = New-Object byte[] (8MB)
...
# 建 Tag、推 Tag、建 Release（含 3 次重試以防網路抖動）
git tag -a $tag -m ("Monthly handover bundle ({0})" -f $tag)
git push origin $tag

$notes = "Handover bundle: $tag`r`n`r`nSHA256: $sha`r`n"
$nf = "RELEASE_NOTES_$tag.txt"; $notes | Out-File $nf -Encoding utf8

$ok=$false
for($i=1;$i -le 3 -and -not $ok;$i++){
  gh release create $tag -R $RepoSlug -t $tag -F $nf --verify-tag --latest $assets
