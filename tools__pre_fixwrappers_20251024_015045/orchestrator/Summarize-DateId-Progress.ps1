...
if(-not $logs){
  Write-Host "（找不到符合的 fullmarket log）"
  exit 0
}
...
foreach($f in $logs){
  ...
  $hdrsRaw = Select-String -Path $f.FullName -Pattern '^===\s+(\d{4}-\d{2}-\d{2})\s+->\s+(\d{4}-\d{2}-\d{2})\s+===' -ErrorAction SilentlyContinue
  $hdrs = @($hdrsRaw)     # <- 強制陣列化，避免只有 1 筆時不是陣列
  ...
  for($i=0; $i -lt $hdrs.Count; $i++){
    ...
    # 是否為「上一個完成窗」：若後面還有下一個 header，代表這個窗已完成（下一窗已開始）
    $isLastCompleted = 0
...
if($lastCompletedEnd){
  ...
  Write-Host "尚無完整完成窗（每份 log 內目前只看到單一 header）。" -ForegroundColor Yellow
}
