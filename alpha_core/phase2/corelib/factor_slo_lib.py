# C:\AI\tw-alpha-stack\alpha_core\factor_slo_lib.py
#!/usr/bin/env python
"""
factor_slo_lib.py
...
def _normalize_factor_map(factors_node: Any) -> Dict[str, Mapping[str, Any]]:
    ...
    if isinstance(factors_node, _Iterable) and not isinstance(factors_node, (str, bytes)):
...
def extract_passed_factors(wf_summary: Mapping[str, Any]) -> Dict[str, Mapping[str, Any]]:
    ...
    def _fallback_windows_payload() -> Dict[str, Any]:
        default_windows = [6, 12, 24]
        overall = wf_summary.get("overall")
        if isinstance(overall, Mapping):
            wins_node = overall.get("windows")
            if isinstance(wins_node, _Iterable) and not isinstance(wins_node, (str, bytes)):
                wins: List[int] = []
                for w in wins_node:
                    try:
        ...
        return {"windows": list(default_windows)}

    factors_map = _normalize_factor_map(wf_summary.get("factors"))

    fbs = wf_summary.get("factors_by_status")
    if isinstance(fbs, Mapping):
        passed_node = fbs.get("passed")
        if passed_node is not None:
            if isinstance(passed_node, Mapping):
                return _normalize_factor_map(passed_node)

            if isinstance(passed_node, _Iterable) and not isinstance(passed_node, (str, bytes)):
                items = list(passed_node)
                if all(isinstance(item, Mapping) for item in items):
                    return _normalize_factor_map(items)

                if all(isinstance(item, str) for item in items):
                    # Canonical compose schema: passed as factor_id list.
                    # Prefer payload from wf_summary.factors to keep window semantics.
                    payload_map: Dict[str, Mapping[str, Any]] = {}
                    fallback_payload = _fallback_windows_payload()
                    if factors_map:
                        for raw in items:
                            fid = raw.strip()
                            if not fid:
                                continue
                            if fid in factors_map:
                                payload_map[fid] = factors_map[fid]
                    else:
                        for raw in items:
                            fid = raw.strip()
                            if not fid:
                                continue
                            payload_map[fid] = dict(fallback_payload)
                    return payload_map
...
def evaluate_factor_slo(
    ...
    # -----------------------------------------------------------------------
    # 先一律計算每個 window 的因子顆數（即使沒有任何門檻，也要有真實統計）
    # -----------------------------------------------------------------------
    from collections.abc import Iterable as _Iterable
    ...
    for fid in factor_ids:
        ...
        for w in win_list:
            ...
            if isinstance(win_node, Mapping):
                ...
                if isinstance(win_node, _Iterable) and not isinstance(win_node, (str, bytes)):
                    for wn in win_node:
                        if str(wn) == str(w):
                            present = True
                            break
    ...
    # -----------------------------------------------------------------------
    # 若完全沒有門檻：dev / 觀察模式 → 回傳資訊性結果，永遠 satisfied=True
    # -----------------------------------------------------------------------
    if not has_any_constraint:
        return FactorSloResult(
            ...
            per_window_counts=dict(per_window_counts),
            missing_required_factors=[],
            satisfied=True,
        )

    # -----------------------------------------------------------------------
    # 有門檻時：套用現有 SLO 規則（global + per-window + required_factors）
    # -----------------------------------------------------------------------
    satisfied = True
