# -*- coding: utf-8 -*-
"""
alpha_core.phase2.corelib.factor_engine
...
- Executing those tasks concurrently using an implementation module
  (by default ``alpha_core.phase2.factor_impl``).
- Writing a JSONL ledger per successful task.
- Writing a single JSON summary file describing the whole batch run.

Public entrypoints:
- :func:`run_factor_engine` – programmatic API used by scripts/p2/factor_engine.py.
...
from .io import load_factor_panel
...
def run_factor_engine(cfg: FactorEngineConfig) -> FactorEngineSummary:
    """
    High-level orchestration entrypoint used by scripts/p2/factor_engine.py.
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--impl-module",
        dest="impl_module",
        default="alpha_core.phase2.factor_impl",
        help="Python module implementing factor computation (default: alpha_core.phase2.factor_impl)",
    )
    parser.add_argument(
        "--factor-root",
        dest="factor_root",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    .. code-block:: bash
        ...
        python -m alpha_core.phase2.corelib.factor_engine \\
    ...
    ``scripts/p2/factor_engine.py`` can also delegate here by importing
