...
"""
alpha_core.phase2.corelib.config
...
- 建立「因子技術視圖」FactorDefinition：
  ...
  - 透過 scripts.p2.factor_registry.load_factor_registry() 取得 raw_specs
    → rules_factors.yaml 的唯一 parser 還是 scripts/p2/factor_registry.py
...
try:
    from scripts.p2.factor_registry import load_factor_registry  # type: ignore[import]
...
class FactorDefinition:
    ...
    注意：
    - 治理欄位（owner / tags / universe 等）仍由 scripts.p2.factor_registry.FactorConfig 負責。
    - 這裡只放「計算 & Gate」需要的資訊。
    """
...
def load_factor_definitions(
    ...
    if load_factor_registry is None:
        raise ConfigError(
            "scripts.p2.factor_registry.load_factor_registry is not available; "
